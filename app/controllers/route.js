...
const RouteModal = require("../modals/route");
...
module.exports = {
  ...
  addRoute: addRoute,
  setRoute: setRoute,
  fetchOneRoute: fetchOneRoute,
  fetchCustomRoute: fetchCustomRoute,
  fetchAllRoute: fetchAllRoute,
  fetchTotalRoute: fetchTotalRoute,
  ...
  RouteCollection.get()
    .then((snapshot) => {
      snapshot.forEach((doc) => {
        const updatedData = RouteModal.returnUpdatedRouteCollectionField(
          doc.data()
        );
...
function fetchOneRoute(req, res) {
  if (req.body.document_id) {
    RouteCollection.doc(req.body.document_id)
      .get()
      .then((response) => {
        if (!response.exists) {
          res.status(400);
...
function fetchCustomRoute(req, res) {
  const params = req.body;
  RouteCollection.where(params.key, params.condition, params.value)
    .get()
    .then((response) => {
      if (response.empty) {
        res.status(400);
...
function fetchAllRoute(req, res) {
  ...
  let Query = RouteCollection;
  if (order_by != null && start_index == null) {
    Query = RouteCollection.orderBy(order_by.key, order_by.value);
  }
  if (order_by != null && start_index != null) {
    Query = RouteCollection.orderBy(order_by.key, order_by.value).startAfter(
      start_index
    );
  }
  Query.limit(limit)
...
function fetchTotalRoute(req, res) {
  RouteCollection.get()
    .then((snapshot) => {
      const finalData = [];
      snapshot.forEach((doc) => {
    ...
    RouteCollection.doc(req.body.document_id.toString())
      ...
      .then((response) => {
        if (!response.exists) {
          ...
          const RouteData = RouteModal.returnUpdatedRouteData(
            response.data(),
            req.body
          );
          RouteCollection.doc(req.body.document_id.toString())
