  ...
  UserCollection.get()
    .then((snapshot) => {
      snapshot.forEach((doc) => {
        const updatedData = UserModal.returnUpdatedUserCollectionField(
          doc.data()
        );
    ...
    UserCollection.doc(parseInt(req.body.mobile).toString())
      ...
      .then((response) => {
        if (response.exists) {
          ...
          if (
            parseInt(usersData["mobile_code"]) == parseInt(req.body.mobile_code)
          ) {
            const updatedData = {
                ...
                const mailData = {
                  ...
                  pdfBase64Data:
                    usersData["user_type"] == "customer"
                      ? null
                      : Mailer.returnVendorAgreementMailer(usersData),
                  pdfName:
                    usersData["user_type"] == "customer"
                      ? null
                      : "Agreement.pdf",
...
function fetchCustomUser(req, res) {
  ...
  UserCollection.where(params.key, params.condition, params.value)
    .get()
    .then((response) => {
      if (response.empty) {
        res.status(400);
    ...
    UserCollection.doc(parseInt(req.body.mobile).toString())
      ...
      .then((response) => {
        if (!response.exists) {
          ...
          const usersData = UserModal.returnUpdatedUserData(
            response.data(),
            req.body
          );
          UserCollection.doc(parseInt(req.body.mobile).toString())
        ...
        if (!response.exists) {
          ...
          if (userData["user_type"] == "customer") {
            ...
            return res.json({
              success: false,
              message:
                "Unable to verify user account, you are a vendor. Please use another account!",
            });
          }
        }
      })
        ...
        if (!response.exists) {
          ...
          if (userData["user_type"] == "customer") {
            ...
            return res.json({
              success: false,
              message:
                "This mobile number is already in use, please use another mobile number!",
            });
          }
        }
      })
        ...
        if (!response.exists) {
          ...
          if (userData["mobile_code"] == req.body.mobile_code) {
            ...
            return res.json({
              success: false,
              message:
                "Invalid verification code, please use correct verification code!",
            });
          }
        }
      })
